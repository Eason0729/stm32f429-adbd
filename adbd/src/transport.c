...
static void dump_packet(const char* name, const char* func, apacket* p)
{
    ...
    D("%s: %s: [%s] arg0=%s arg1=%s (len=%d) ", name, func, cmd, arg0, arg1, len);
    dump_hex(p->data, len);
}
#endif /* ADB_TRACE */
...
void send_packet(apacket* p, atransport* t)
{
    ...
    if(write_packet(t->transport_write, t->serial, &p)) {
...
static void* output_thread(void* _t)
{
    ...
    D("%s: starting transport output thread on fd %d, SYNC online (%d)\n",
      t->serial, t->fd_write, t->sync_token + 1);
    p = get_apacket();
    p->msg.command = A_SYNC;
    p->msg.arg0 = 1;
    p->msg.arg1 = ++(t->sync_token);
    p->msg.magic = A_SYNC ^ 0xffffffff;
    if(write_packet(t->fd_write, t->serial, &p)) {
        put_apacket(p);
...
static void* input_thread(void* _t)
{
    ...
    for(;;) {
        if(read_packet(t->fd_read, t->serial, &p)) {
  ...
  // closes while a client socket is still active.
    close_all_sockets(t);
...
static int device_tracker_send(device_tracker* tracker, const char* buffer,
                               int len)
{
...
static void device_tracker_ready(asocket* socket)
{
  ...
  /* we want to send the device list when the tracker connects
    ...
    if(tracker->update_needed > 0) {
        ...
        len = list_transports_msg(buffer, 1024);
...
static void transport_registration_func(int _fd, unsigned ev, void* data)
{
    ...
    if(m.action == 0) {
        D("transport: %s removing\n", t->serial);

    /* IMPORTANT: the remove closes the transport
         ** pipes.
         */
        close_all_sockets(t);
        fdevent_remove(&(t->transport_fde));
        adb_close(t->fd_read);
        adb_close(t->fd_write);
        adb_close(t->transport_read);
        adb_close(t->transport_write);
   ...
   /* don't create transport threads for inaccessible devices */
    ...
    /* initial references are the two threads */
        ...
        int io_to_main[2], main_to_io[2];
        if(pipe(io_to_main) < 0 || pipe(main_to_io) < 0) {
            fatal_errno("cannot open transport pipes");
        }

        close_on_exec(io_to_main[0]);
        close_on_exec(io_to_main[1]);
        close_on_exec(main_to_io[0]);
        close_on_exec(main_to_io[1]);

        D("transport: %s starting (fd_read=%d fd_write=%d transport_read=%d transport_write=%d)\n",
          t->serial, main_to_io[0], io_to_main[1], io_to_main[0], main_to_io[1]);

        t->fd_read = main_to_io[0];
        t->fd_write = io_to_main[1];
        t->transport_read = io_to_main[0];
        t->transport_write = main_to_io[1];

        fdevent_install(&(t->transport_fde), t->transport_read,
...
void init_transport_registration(void)
{
    ...
    transport_registration_send = s[1];
    transport_registration_recv = s[0];
...
static void add_qual(char** buf, size_t* buf_size, const char* prefix,
                     const char* qual, int sanitize_qual)
{
    size_t len;
    int prefix_len;

    if(!buf || !*buf || !buf_size || !*buf_size || !qual || !*qual) return;
    ...
    if(sanitize_qual) {
        ...
        for(cp = *buf + prefix_len; cp < *buf + len; cp++) {
            if(qual_char_is_invalid(*cp)) *cp = '_';
        }
    }
