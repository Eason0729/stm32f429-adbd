...
int local_name_to_fd(const char* name)
{
  ...
  // It's non-sensical to support the "reserved" space on the adb host side
    if(!strncmp(name, "local:", 6)) {
        ...
        return socket_local_server(name + 16, ANDROID_SOCKET_NAMESPACE_FILESYSTEM,
                                   SOCK_STREAM);
    }
...
static install_status_t install_listener(const char* local_name,
                                         ...
                                         atransport* transport, int no_rebind)
{
    alistener* l;
...
int launch_server(int server_port)
{
  ...
  // child.
    ...
    if(pid == 0) {
    // child side — must not modify path after vfork
        adb_close(fd[0]);
        dup2(fd[1], STDERR_FILENO);
        adb_close(fd[1]);

        execl(path, "adb", "-P", str_port, "fork-server", "server", NULL);
        write(STDERR_FILENO, "OOPS! execl failed\n", 19);
        _exit(127);
    } else {
    // parent side
        char temp[3];
...
int adb_main(int is_daemon, int server_port)
{
...
#elif defined(HAVE_FORKEXEC) || defined(HAVE_VFORKEXEC)
  ...
  /* don't run as root if we are running in secure mode */
    ...
    // check for locked password
        if(spwd->sp_pwdp[0] == '!') {
            fprintf(stderr,
                    "user: %s, password locked, please unlock account first\n",
                    pw->pw_name);
...
void connect_emulator(char* port_spec, char* buffer, int buffer_size)
{
    ...
    if(!port_separator) {
        snprintf(buffer, buffer_size,
                 "unable to parse '%s' as <console port>,<adb port>", port_spec);
        return;
    }
  ...
  /* Check if the emulator is already known.
    ...
    if(known_emulator != NULL) {
        snprintf(buffer, buffer_size, "Emulator on port %d already registered.",
                 adb_port);
        return;
    }
  ...
  /* Preconditions met, try to connect to the emulator. */
    if(!local_connect_arbitrary_ports(console_port, adb_port)) {
        snprintf(buffer, buffer_size, "Connected to emulator on ports %d,%d",
                        ...
                        int reply_fd, asocket* s)
{
    ...
    if(!buf) {
        sendfailmsg(reply_fd, "out of memory");
        return -1;
    }
  ...
  // transport "transport-any:" is used for switching transport to the only
  // transport
  ...
  // return a list of all connected devices
    if(!strncmp(service, "devices", 7)) {
        ...
        if(use_long || service[7] == 0) {
            ...
            list_transports(buffer, 4096, use_long);
            snprintf(buf, 4096, "OKAY%04x%s", (unsigned)strlen(buffer),
                     buffer);
            D("Wrote device list \n");
  ...
  // add a new TCP transport, device or emulator
    if(!strncmp(service, "connect:", 8)) {
        ...
        if(!strncmp(host, "emu:", 4)) {
            connect_emulator(host + 4, buffer, 4096);
        } else {
            connect_device(host, buffer, 4096);
        }
    // Send response for emulator and device
        snprintf(buf, 4096, "OKAY%04x%s", (unsigned)strlen(buffer), buffer);
        writex(reply_fd, buf, strlen(buf));
        free(buffer);
        ret = 0;
        goto cleanup;
    ...
    if(!strncmp(service, "disconnect:", 11)) {
      ...
      // assume port 5555 if no port is specified
            ...
            if(t) {
                ...
                snprintf(buffer, 4096, "No such device %s", serial);
            }
        }
  ...
  // returns our value for ADB_SERVER_VERSION
    if(!strcmp(service, "version")) {
        ...
        snprintf(buf, 4096, "OKAY%04x%s", (unsigned)strlen(version), version);
        writex(reply_fd, buf, strlen(buf));
        ret = 0;
        goto cleanup;
    }
    ...
    if(!strncmp(service, "get-serialno", strlen("get-serialno"))) {
        ...
        snprintf(buf, 4096, "OKAY%04x%s", (unsigned)strlen(out), out);
        writex(reply_fd, buf, strlen(buf));
        ret = 0;
        goto cleanup;
    }
  ...
  // indicates a new emulator instance has started
    ...
    if(!strncmp(service, "get-state", strlen("get-state"))) {
        ...
        snprintf(buf, 4096, "OKAY%04x%s", (unsigned)strlen(state), state);
        writex(reply_fd, buf, strlen(buf));
        ret = 0;
        goto cleanup;
    }
