...
struct asocket* asocket_init(int fd)
{
  ...
  /* set primary socket to non-blocking */
    ...
    if(fcntl(fd, F_SETFL, flags | O_NONBLOCK)) return NULL;

  /* create pipe with non-blocking write, so that asocket_close() cannot
                   block */
    if(pipe(abort_fd)) return NULL;
    flags = fcntl(abort_fd[1], F_GETFL);
    if(flags == -1) return NULL;
    if(fcntl(abort_fd[1], F_SETFL, flags | O_NONBLOCK)) return NULL;
    ...
    /* ready to poll() */
      ...
      /* abort due to asocket_abort() */
        ...
        /* connect call complete, read return code */
                retlen = sizeof(ret);
                if(getsockopt(s->fd, SOL_SOCKET, SO_ERROR, &ret, &retlen)) return -1;
...
int asocket_accept(struct asocket* s, struct sockaddr* addr, socklen_t* addrlen,
                   int timeout)
{
    int ret;
    struct pollfd pfd[2];
