...
static void dump_fde(fdevent* fde, const char* info)
{
    ...
    fprintf(stderr, "FDE #%03d %c%c%c %s\n", fde->fd,
            fde->state & FDE_READ ? 'R' : ' ', fde->state & FDE_WRITE ? 'W' : ' ',
...
{
    struct epoll_event* events = malloc(256 * sizeof(struct epoll_event));
    fdevent* fde;
    int i, n;
...
static void dump_all_fds(const char* extra_msg)
{
    int i;
    fdevent* fde;
    size_t size = FD_SETSIZE * 6 + 1;
  // per fd: 4 digits (but really: log10(FD_SETSIZE)), 1 staus, 1 blank
    char* msg_buff = malloc(size);
    char* pb;
    size_t max_chars;
    int printed_out;

    if(msg_buff == 0) return;
    pb = msg_buff;
    max_chars = size;
...
void fdevent_subproc_setup()
{
    ...
    SHELL_EXIT_NOTIFY_FD = s[1];
    fdevent* fde;
    fde = fdevent_create(s[0], fdevent_subproc_event_func, NULL);
