...
int adb_b64_ntop(src, srclength, target, targsize)
u_char const* src;
size_t srclength;
char* target;
size_t targsize;
{
    ...
    while(2 < srclength) {
        ...
        output[1] =
            ((u_int32_t)(input[0] & 0x03) << 4) + ((u_int32_t)input[1] >> 4);
        output[2] =
            ((u_int32_t)(input[1] & 0x0f) << 2) + ((u_int32_t)input[2] >> 6);
...
int adb_b64_pton(src, target, targsize)
char const* src;
u_char* target;
size_t targsize;
{
