...
static int fail_message(int s, const char* reason)
{
    ...
    if(writex(s, &msg.data, sizeof(msg.data)) || writex(s, reason, len)) {
        return -1;
    } else {
        return 0;
    }
...
static int handle_send_file(int rfd, int wfd, char* path, mode_t mode, char* buffer)
{
    ...
    for(;;) {
        ...
        if(readx(rfd, &msg.data, sizeof(msg.data))) goto fail;
        ...
        if(len > SYNC_DATA_MAX) {
            fail_message(wfd, "oversize data message");
            goto fail;
        }
        if(readx(rfd, buffer, len)) goto fail;
    ...
    if(fd >= 0) {
        ...
        msg.status.msglen = 0;
        if(writex(wfd, &msg.status, sizeof(msg.status))) return -1;
    }
    return 0;
...
#ifdef HAVE_SYMLINKS
static int handle_send_link(int rfd, int wfd, char* path, char* buffer)
{
    syncmsg msg;
    unsigned int len;
    int ret;

    if(readx(rfd, &msg.data, sizeof(msg.data))) return -1;

    if(msg.data.id != ID_DATA) {
        fail_message(wfd, "invalid data message: expected ID_DATA");
        return -1;
    }
    ...
    if(readx(rfd, buffer, len)) return -1;
    ...
    if(msg.data.id == ID_DONE) {
        ...
        fail_message(wfd, "invalid data message: expected ID_DONE");
        return -1;
    }
...
#ifdef HAVE_SYMLINKS
    if(is_link)
        ret = handle_send_link(rfd, wfd, path, buffer);
    else {
#else
    {
#endif
...
void file_sync_service(int rfd, int wfd, void* cookie)
{
    syncmsg msg;
    char* name = NULL;
    unsigned namelen;
    ...
    for(;;) {
        ...
        if(readx(rfd, &msg.req, sizeof(msg.req))) {
        ...
        switch(msg.req.id) {
        case ID_STAT:
            if(do_stat(wfd, name)) goto fail;
            break;
        case ID_LIST:
            if(do_list(wfd, name)) goto fail;
            break;
        case ID_SEND:
            if(do_send(rfd, wfd, name, buffer)) goto fail;
            break;
        case ID_RECV:
            if(do_recv(wfd, name, buffer)) goto fail;
            break;
        case ID_QUIT:
            goto fail;
        default:
