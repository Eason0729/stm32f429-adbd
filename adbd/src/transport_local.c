 ...
 * data between the host, and the guest. See external/qemu/android/adb-*.* that
 * implements the emulator's side of the protocol. Another advantage of using
 * QEMUD approach is that ADB will be up much sooner, since it doesn't depend
 * anymore on network being set up.
 * The guest side of the protocol contains the following phases:
 * - Connect with adb QEMUD service. In this phase a handle to 'adb' QEMUD
 * service is opened, and it becomes clear whether or not emulator supports that
 *   protocol.
 ...
 * the adb QEMUD service, indicating that the guest is ready to receive
 * messages. Note that the guest will ignore messages sent down from the
 * emulator before the transport registration is completed. That's why we need
 * to send the 'start' request after the transport is registered.
 */
