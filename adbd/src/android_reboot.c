...
{
    FILE* f;
    char* mount_dev = malloc(256);
    char* mount_dir = malloc(256);
    char* mount_type = malloc(256);
    char* mount_opts = malloc(256);
    int mount_freq;
    int mount_passno;
    int match;
    int found_rw_fs = 0;

    if(!mount_dev || !mount_dir || !mount_type || !mount_opts) {
        free(mount_dev);
        free(mount_dir);
        free(mount_type);
        free(mount_opts);
    ...
    do {
        match = fscanf(f, "%255s %255s %255s %255s %d %d\n", mount_dev, mount_dir,
                       mount_type, mount_opts, &mount_freq, &mount_passno);
        mount_dev[255] = 0;
        mount_dir[255] = 0;
        mount_type[255] = 0;
        mount_opts[255] = 0;
...
int android_reboot(int cmd, int flags, char* arg)
{
    ...
    if(!(flags & ANDROID_RB_FLAG_NO_SYNC)) sync();

    if(!(flags & ANDROID_RB_FLAG_NO_REMOUNT_RO)) remount_ro();
