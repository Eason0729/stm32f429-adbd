...
static int read_central_dir_values(Zipfile* file, const unsigned char* buf,
                                   int len)
{
    if(len < EOCD_LEN) {
                                        ...
                                        ssize_t* len)
{
  ...
  // but the one in the local file header also contains some padding.
    ...
    dataOffset = localHeaderRelOffset + LFH_SIZE + entry->fileNameLength +
                 extraFieldLength;
...
int read_central_dir(Zipfile* file)
{
  ...
  // extract eocd values
    ...
    if(file->disknum != 0 || file->diskWithCentralDir != 0 ||
