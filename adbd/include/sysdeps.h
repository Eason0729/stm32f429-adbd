...
static __inline__ void close_on_exec(int fd) { /* nothing really */ }
...
static __inline__ int unix_close(int fd) { return close(fd); }
#undef close
...
static __inline__ void adb_sleep_ms(int mseconds) { Sleep(mseconds); }
...
static __inline__ int unix_socketpair(int sv[2])
{
    int rc = pipe(sv);
    if(rc < 0) return -1;
    return 0;
