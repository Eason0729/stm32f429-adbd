...
#define ADB_SERVER_VERSION \
    31  // Increment this when we want to force users to start a new adb server
...
#if !ADB_HOST
typedef enum { BACKUP,
               RESTORE } BackupOperation;
int backup_service(BackupOperation operation, char* args, int out_fds[2]);
void framebuffer_service(int rfd, int wfd, void* cookie);
void log_service(int rfd, int wfd, void* cookie);
void remount_service(int rfd, int wfd, void* cookie);
